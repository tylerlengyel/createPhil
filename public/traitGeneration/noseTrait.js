...
/**
 * Generates a basic Nose trait SVG.
 * If no JSON data is provided, it automatically fetches the file
 * /traits_json/nose_common.json (or another rarity if specified).
 *
...
export async function generateTrait(rarity = "common", jsonData) {
  // If no JSON data was provided, fetch the default file.
  if (!jsonData) {
    jsonData = await fetchJSON(`/traits_json/nose_${rarity}.json`);
  }
  
  if (!jsonData.pathData) {
    throw new Error("Missing pathData for Nose trait.");
  }
